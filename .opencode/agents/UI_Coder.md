---
description: 高品质 UI 呈现层实现者。负责布局、样式、视觉层次、响应式、交互反馈与无障碍呈现。完成实现后必须同步相关 .Nexus/0-fact/ 并写实现情况文档。不负责业务逻辑实现。
mode: subagent
permission:
  bash:
    "": ask
    "mkdir -p .Nexus/0-fact/*": allow
    "mkdir -p .Nexus/3-implement": allow
  read:
    "**": allow
  write:
    "**": allow
