---
description: 研究者。负责产出架构级方案、功能级预研方案、功能级步骤文档。优先从 .Nexus/0-fact/ 获取事实，必要时读取真实代码核对。按契约按需读取对应流程 skill。
mode: subagent
permission:
  bash:
    "": ask
    "mkdir -p .Nexus/1-research": allow
    "mkdir -p .Nexus/2-Scheme": allow
    "mkdir -p .Nexus/.tool": allow
  read:
    "**": allow
  write:
