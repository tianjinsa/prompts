...
7. **实现报告落盘**：除聊天回复外，你必须将完整的 `Implementation Report` 写入 Master 提供的 `Implementation Report Path`。这份文件是下游 agent 和会话恢复流程可读取的权威交接材料。若缺少该路径，立即停止并向 Master 返回阻碍。
---
...
- **接口交接是强制要求**：在你的实现报告中，必须以标准化格式（见报告格式中 `Interfaces Exposed for UI_Coder` 部分）完整列出所有暴露给 UI 层的接口——包括类型签名、用途说明和最小使用示例。这是 `UI_Coder` 能否顺利工作的前提。
- **实现报告落盘**：每次实现完成后，必须将完整的 Implementation Report 同步写入 `Implementation Report Path`（即 `.Nexus/2-implementation/[yymmdd]_[task-slug]_coder.md`）。聊天回复中必须包含该报告路径。
...
[列出为 UI 层暴露的接口，或 "None — 此任务不涉及 UI 集成"]

如果存在暴露的接口，必须按以下固定格式列出每个接口：

#### `[接口名称]`（hook / state / callback / type / context）
- **文件路径**: `path/to/file.ts`
- **类型签名**: 
  ```typescript
  // 完整的类型签名
  ```
- **用途**: [一句话说明 UI_Coder 应如何使用此接口]
- **输入**: [参数含义和约束]
- **输出/返回值**: [返回值结构和含义，包括 loading/error 状态]
- **使用示例**:
  ```typescript
  // 最小使用示例
  ```
- **注意事项**: [可空性、异步时序、依赖前置条件等]
