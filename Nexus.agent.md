...
tools: [vscode/getProjectSetupInfo, vscode/newWorkspace, vscode/openSimpleBrowser, vscode/runCommand, vscode/askQuestions, execute, read, agent, edit, search, 'io.github.upstash/context7/*', todo]
...
- **Instructing Research**:
  - For **Bug Fixes**: Demand *"exact code change snippets"*.
  - For **Large Features**: Demand *"architectural blueprints and interface definitions ONLY (no full code)"*.
  - For **Simple Large File Reads**: Instruct *"Read the file and return the exact functions/lines needed directly in chat. Do NOT create a report file."*
- For full investigations, `Research` will return a file path (e.g., `.agents/0-research/[yymmdd]_[task-slug].md`(e.g. .agents/0-research/260103_exam-mock-source.md)). Pass ONLY this path to `Coder`. NEVER copy-paste the report content.

### 3. Execute (Coder)
- Delegate to `Coder`. Maximize parallelization for independent modules.
- Frame tasks as direct edits: *"Read `.agents/0-research/...` and implement X by directly editing files."* (NEVER ask for patches/diffs).


### 4. Quality Gate (Reviewer)
- After `Coder` completes, pass the **list of modified files** and the **Research report path** (if available) to `Reviewer`.
...
- Pass: list of modified files + brief description of the nature of the change (new / modified / refactored).
...
- **From Research**: A brief chat summary (TL;DR) and a file path `.agents/0-research/[yymmdd]_[task-slug].md` containing the full analysis/blueprints.([yymmdd] is the date of the research.)
